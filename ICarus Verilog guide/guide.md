...
# Дальше ниче не работает)))) Потом перепишу((())
...
командную строку. Все на примере MacOS, bash-script под другие системы, возможно, придется
переписать, это не сложно, сами справитесь.

### Поддержка правописания языка Verilog
Для этого нужно установить плагин `SystemVerilog`[(ссылка)][sys_verilog] или 
более новый `Verilog Language Support`[(ссылка)][ver_lang_sup] (первым я пользовался, работает 
...
<img src="pictures/run_configs_final.png">
...
и это не очень удобно :(. Чтобы это исправить, тыкаем правой кнопкой мыши на файле и выбираем 
...
Есть нюанс: придется каждый раз при создании нового проекта добавлять по нашему шаблону такой
вид запуска. Как это исправить - хз. 
